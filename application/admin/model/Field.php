...
class Field extends Model
{
    ...
    public function field($param, $dataInfo = [])
    {
        ...
        if ($param['action'] == 'index' || $param['action'] == 'pool') {
            ...
            $field_list = $this->where($map)->field('field,types,name,form_type,default_value,is_unique,is_null,is_automatic_number,input_tips,setting')->order($order)->select();
            //客户
    ...
    public function validateField($types, $types_id = 0)
    {
        ...
        $fieldList = $this->where(['types' => ['in',['',$types]],'types_id' => $types_id,'field' => ['not in',$unField],'form_type' => ['not in',['checkbox','user','structure','file']]])->field('field,name,form_type,is_unique,is_null,is_automatic_number,max_length')->select();
        $validateArr = [];
        $rule = [];
        $message = [];
        ...
        if ($userFieldData) {
            ...
            foreach ($userFieldData as $k=>$v) {
                if (empty($v['is_hide'])) {
                    ...
                    $fieldArr[$i]['is_automatic_number'] = $fieldList[$k]['is_automatic_number'];
